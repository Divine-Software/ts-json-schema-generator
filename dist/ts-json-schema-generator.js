...
try {
    const typeMaps = [];
    const schema = (0, generator_1.createGenerator)(config).createSchema(args.type, typeMaps);
...
function writeTypeMapFile(typeMaps, typeMapeFile) {
    ...
    typeMaps.forEach((typeMap) => typeMap.typeNames.forEach((typeName) => {
        code += `    [\`${typeName}\`]: ${typeName};\n`;
    }));
    code += `}\n`;
    (0, fs_1.mkdirSync)(typeMapDir, { recursive: true });
    (0, fs_1.writeFileSync)(typeMapeFile, code);
}
